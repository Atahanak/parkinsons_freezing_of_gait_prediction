...
fog_train_loader = DataLoader(fog_train, batch_size=cfg["batch_size"], shuffle=True) #, num_workers=1)
fog_train_loader_2 = DataLoader(fog_train_2, batch_size=cfg["batch_size"], shuffle=True) #, num_workers=1)
fog_valid = FOGDataset(valid_fpaths, cfg, task = 'seperate')
fog_valid_loader = DataLoader(fog_valid, batch_size=cfg["batch_size"]) #, num_workers=1)
...
def train_model(module, model, head, train_loaders, val_loader, test_loader, save_name = None, **kwargs):
    """
    Inputs:
...
model, trainer, result = train_model(FOGFinetuneModule, model, head, [fog_train_loader, fog_train_loader_2], fog_valid_loader, fog_valid_loader, save_name="FOGEventSeperator", optimizer_name="Adam")
